...
fn buildDefault(allocator: std.mem.Allocator, state: *State, buf: []u8) []const u8 {
    ...
    const branch_suffix = if (getGitBranch(allocator, state)) |b|
        std.fmt.bufPrint(buf[1024..1280], ansi.magenta ++ "({s})" ++ ansi.reset ++ " ", .{b}) catch ""
    else
        "";
...
fn getGitBranch(allocator: std.mem.Allocator, state: *State) ?[]const u8 {
    ...
    return if (branch.len > 0) branch else null;
}
